...
> We estimate the effect of weather anomalies on Mexico's total GDP per capita growth and headline inflation. We extend our analysis by looking at three productive sectors (primary, secondary, and tertiary) and five sub-indices of inflation (services, non-food items, food items, agriculture, and energy). The analysis draws on data from 32 federal entities in Mexico aggregated in seven geographical regions. To estimate the effects of weather anomalies, we combine two methodologies: a Panel Autoregressive Distributed Lag Model for GDP per capita growth and Panel Local Projections for inflation. Our results indicate that neither temperature nor precipitation anomalies have a statistically significant effect on total GDP per capita,  headline inflation, and inflation sub-indices. We find that weather anomalies produce significant long-term effects on real GDP per capita growth primarily in the primary sector, where both positive and negative temperature deviations reduce growth, with an annual increase of 0.01°C above the historical norm reducing growth by approximately 0.01 percentage points per year. Long-term effects on the secondary sector are limited to precipitation anomalies and are negligible in magnitude.
